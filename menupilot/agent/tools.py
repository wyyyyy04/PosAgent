...
# ── 模板配置 ──────────────────────────────────────────────────────
# 不同模板的表头行号不同。header_row=列头所在行, data_start_row=数据起始行。

TEMPLATE_CONFIG = {
    "tasty": {
        "header_row": 2,        # Row 2 是真实列头（Row 1 是合并标题，Row 3 是说明）
        "data_start_row": 4,    # 跳过 Row 3 说明行，从 Row 4 开始写数据
    },
}
...
def run_option_expansion(
    ...
    try:
        ...
        # 按模板名取配置（从 template_path 提取模板名）
        tpl_name = "tasty" if "tasty" in template_path.lower() else ""
        tpl_cfg = TEMPLATE_CONFIG.get(tpl_name, {})
        write_expanded_template(
            template_path, output_path, expanded_df,
            header_row=tpl_cfg.get("header_row", 2),
            data_start_row=tpl_cfg.get("data_start_row", 4),
        )
